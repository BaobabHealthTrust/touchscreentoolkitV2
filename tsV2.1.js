...
function loadPage(section, back) {
    ...
    if (__$("legend")) {
        ...
        __$("legend").innerHTML = fieldsets[section].getElementsByTagName("legend")[0].innerHTML;
            ...
            for (var j = 0; j < 4; j++) {
                ...
                switch (j) {
                    case 0:
                        ...
                        var label = addLabel(cell, (fields[i].label != undefined ? fields[i].label.innerHTML : "Undefined"), textSize, "#333");
    ...
    if (fieldtype != null) {
        ...
        switch (fieldtype.toLowerCase()) {
            case "age":
                ...
                addAge(__$("toolsCell2"), __$("textFor" + fields[pos].id), (!isNaN((new Date(__$("textFor" + fields[pos].id).value.trim())).getDay()) ? __$("textFor" + fields[pos].id).value.trim() : undefined), "Specify Actual Date of Birth", "Age");
            ...
            case "date":
                ...
                var ctrl = addDate(__$("toolsCell2"), __$("textFor" + fields[pos].id), __$("textFor" + fields[pos].id).value);
...
function gotoQuestion(pos, section) {
    ...
    if (fields[pos].getAttribute("optional") == null) {

        if (__$("textFor" + fields[pos].id)) {

            if (__$("textFor" + fields[pos].id).value.trim().length == 0) {
    ...
    if (fields[pos].getAttribute("absolute_max") != null) {

        if (__$("textFor" + fields[pos].id)) {

            if (eval(__$("textFor" + fields[pos].id).value.trim()) > eval(fields[pos].getAttribute("absolute_max"))) {

                incomplete = true;

                alert("Value greater that maximum allowable!");
    ...
    if (fields[pos].getAttribute("max") != null) {

        if (__$("textFor" + fields[pos].id)) {

            if (eval(__$("textFor" + fields[pos].id).value.trim()) > eval(fields[pos].getAttribute("max"))) {

                incomplete = true;

                if (!confirm("The value is greater than expected! Keep same value?")) {
    ...
    if (fields[pos].getAttribute("absolute_min") != null) {

        if (__$("textFor" + fields[pos].id)) {

            if (eval(__$("textFor" + fields[pos].id).value.trim()) < eval(fields[pos].getAttribute("absolute_min"))) {

                incomplete = true;

                alert("Value greater that minimum allowable!");
    ...
    if (fields[pos].getAttribute("min") != null) {

        if (__$("textFor" + fields[pos].id)) {

            if (eval(__$("textFor" + fields[pos].id).value.trim()) < eval(fields[pos].getAttribute("min"))) {

                incomplete = true;

                if (!confirm("The value is less than expected! Keep same value?")) {
    ...
    if (fields[pos].getAttribute("regex") != null) {

        if (__$("textFor" + fields[pos].id)) {

            var re = RegExp(fields[pos].getAttribute("regex"));
            __$("textForchild_birth_weight").value.search(re)

            if (__$("textFor" + fields[pos].id).value.trim().search(re) == -1) {

                incomplete = true;

                if (fields[pos].getAttribute("regex_message") != null) {

                    alert(fields[pos].getAttribute("regex_message"));
...
function getValidPreviousValue(pos, section) {
    ...
    if (new_pos == undefined && new_section == undefined) {
        ...
        new_section = (section > 0 ? section - 1 : 0);
...
function checkValidity() {
    ...
    validityTmr = setTimeout("checkValidity()", 200);
